...
def plot_sig_annotation(sig, annList, title=None, color='blue', qrsmark='r+',is_tansparent=False):
    ...
    plt.plot(sig, color, linewidth=2)
...
# 获取一个序列中方差最小的num个等比序列
def find_max_minVar_dengbi(index, num):
    dis = np.diff(index)
    dengbi=[]
    for i in range(1,len(dis)):
        dengbi.append(dis[i]/dis[i-1])
    # print(dis)
    min_var = np.var(dis)
    min_var_num = len(dis)
    start_pos = 0
    min_num = num
    for now_num in range(min_num, len(dengbi)):
        for iteration in range(len(dengbi) - now_num):
            now_var = np.var(dis[iteration:iteration + now_num])
            # print(min_var,now_var,now_num)
            if now_var < min_var:
                min_var = now_var
...
def plot_sig_annotation_mark_first(sig, win, annList, first=True, title=None, color='blue', qrsmark='r+'):
    sig = get_max_abs(sig)
    annArr = np.array([None] * len(sig)).T
    for i in range(len(annList)):
        annArr[annList[i]] = sig[annList[i]]
    ...
    fig.patch.set_alpha(0.)
    plt.subplot(111)
    plt.plot(sig, color, linewidth=1.5)
    plt.axis('off')
    plt.plot(win, 'brown')
    if first:
        annArr1 = np.array([None] * len(sig)).T
        annArr1[annList[0]] = sig[annList[0]]
        plt.plot(annArr, 'r+',linewidth=0.8)
        # plt.plot(annArr1, 'g+',linewidth=0.8)
    else:
        annArr2 = np.array([None] * len(sig)).T
        annArr2[annList[-1]] = sig[annList[-1]]
        plt.plot(annArr, 'r+',linewidth=0.8)
        plt.plot(annArr2, 'g+',linewidth=0.8)
    if title != None:
        plt.title(title)
    # plt.axis('off')
    plt.show()
...
def detec_F_2_gravity(Sig, index, num, is_print=False,show_rec=False):
    sig = Sig.copy()  #
    sig_weighted = Sig.copy()  # 这两个都是Sig的副本，第一个用来预测过程中处理，第二个表示加权后的，其实可以弄成一个，但是懒得改

    start, end = find_max_minVar(index, num)  # 找最好的片段，方差最小
    if is_print:
        plot_sig_annotation(check_polarity(Sig, F=True), index, color='g', title='最初片段')
        # np.save('temp.mat',sig[5200:7800])
        plot_sig_annotation(check_polarity(Sig, F=True), index[start:end], color='blue')
    index_start=start
    index_end=end
    pos_num = 0
    neg_num = 0
    ...
    mean_dis = int(np.mean(np.diff(now_index)))
    side_dis = int(mean_dis / 3)
    weighting_dis = int(side_dis * 1.6)

    mean_height = 0
    # 前向推测
    first_weighting = None
    win = []
    ...
    weighting = win[mean_dis - weighting_dis:mean_dis + weighting_dis]
    while now_index[0] - mean_dis - side_dis >= 0:
        if mean_dis < 250:
            # print('推出了')
            break
        ...
        for i in range(len(now_index)):
            mean_height_list.append(abs(Sig[now_index[i]]))
        mean_height = np.mean(mean_height_list)
        pred = now_index[0] - mean_dis # 预测位置
        ...
        for i in range(mean_dis * 2):
            win.append(M / (M + ((i - mean_dis) / (mean_dis / 8)) * ((i - mean_dis) / (mean_dis / 8))))
        weighting = win[mean_dis - weighting_dis:mean_dis + weighting_dis]
        # 当前预测位置的weighting win
        win_arr = np.array([None] * len(sig)).T

        if is_print:
            for i in range(2 * weighting_dis):
                if pred- weighting_dis>=0:
                    win_arr[pred+11- weighting_dis + i] = win[mean_dis-weighting_dis+i]
        else:
            for i in range(2 * weighting_dis):
                if pred - weighting_dis >= 0:
                    win_arr[pred  - weighting_dis + i] = win[mean_dis - weighting_dis + i]
        # np.save('temp.npy', win_arr,allow_pickle=True)
        # print('前向判断f_w:',first_weighting)
        if first_weighting == None:
            # print('执行了前向')
        ...
        try:
            if abs(Sig[precise_loc]) < 0.3 * mean_height:
                ...
                weighting = win[mean_dis * 2 - weighting_dis:mean_dis * 2 + weighting_dis]
        ...
        #  对FECG使用窗函数加权去噪
        if pred >= weighting_dis:
            sig_weighted[pred - weighting_dis:pred + weighting_dis] = np.multiply(
                sig_weighted[pred - weighting_dis:pred + weighting_dis], weighting)
        else:
            sig_weighted[0:pred + weighting_dis] = np.multiply(sig_weighted[0:pred + weighting_dis],
                                                               weighting[weighting_dis - pred:])

        now_index.insert(0, precise_loc)
        mean_dis = int(np.mean(np.diff(now_index[0:num])))
        if is_print:

            plot_sig_annotation_mark_first(check_polarity(Sig, F=True), win_arr, now_index[1:], first=True, color='blue')


            # print(sig_weighted[now_qrs - weighting_dis:now_qrs + weighting_dis].shape)
            # print(len(first_weighting))
    # 补充第一个
    end = now_index[0] - mean_dis + side_dis
    if 0 < end < 2 * side_dis:
        win = signalTool.windows.gaussian(side_dis * 2, std=int(mean_dis / 5))
    ...
    while now_index[-1] + mean_dis + side_dis <= len(sig):
        ...
        for i in range(len(now_index)):
            mean_height.append(abs(Sig[now_index[i]]))
        mean_height = np.mean(mean_height)
        pred = now_index[-1] + mean_dis  # 预测位置
        ...
        win_arr = np.array([None] * len(sig)).T
        for i in range(2 * weighting_dis):
            if pred - weighting_dis + i<len(sig):
                win_arr[pred - weighting_dis + i] = win[mean_dis - weighting_dis + i]
        # print('后向判断f_w:', first_weighting)
        if first_weighting == None:
            # print('执行了后向')
            first_weighting = weighting
        win = win[mean_dis - side_dis:mean_dis + side_dis]
        ...
        #  对FECG使用窗函数加权去噪
        if pred + weighting_dis <= len(sig):
            sig_weighted[pred - weighting_dis:pred + weighting_dis] = np.multiply(
                sig_weighted[pred - weighting_dis:pred + weighting_dis], weighting)
        else:
            sig_weighted[pred - weighting_dis:] = np.multiply(sig_weighted[pred - weighting_dis:],
                                                              weighting[0:len(sig) - pred + weighting_dis])
        ...
        if is_print:
            print(M)
            plot_sig_annotation_mark_first(check_polarity(Sig, F=True), win_arr, now_index, first=False, color='g')
        mean_dis = int(np.mean(np.diff(now_index[-1 * num:])))
    try:
        # 对初始5个良好片段加权
        for i in range(index_start, index_end):
            now_qrs = index[i]
            if now_qrs < weighting_dis:
                sig_weighted[0:now_qrs + weighting_dis] = np.multiply(sig_weighted[0:now_qrs + weighting_dis],
                                                                      first_weighting[weighting_dis - now_qrs:])

            elif now_qrs + weighting_dis > len(sig):
                sig_weighted[now_qrs - weighting_dis:] = np.multiply(sig_weighted[now_qrs - weighting_dis:],
                                                                     first_weighting[
                                                                     0:len(sig) - now_qrs + weighting_dis])
            else:
                # print(sig_weighted[now_qrs - weighting_dis:now_qrs + weighting_dis])
                # print('嘿嘿',first_weighting)
                sig_weighted[now_qrs - weighting_dis:now_qrs + weighting_dis] = np.multiply(
                    sig_weighted[now_qrs - weighting_dis:now_qrs + weighting_dis], first_weighting)
    except:
        pass
    ...
    if now_index[0]>weighting_dis:
        sig_weighted[0:now_index[0]-int(weighting_dis*0.8)]*=weighting[0]
    ...
    if len(sig) - now_index[-1] > dis08:
        ...
        if math.fabs(sig[pos]) > mean * 0.3:
            now_index.insert(len(now_index), pos)
    if now_index[-1]+weighting_dis<len(sig):
        sig_weighted[now_index[-1]+weighting_dis:]*=weighting[-1]


    # 1-27修改
    mean_height=0
    for i in range(len(now_index)):
        mean_height+=abs(sig_weighted[now_index[i]])
    mean_height/=len(now_index)

    for i in range(len(now_index)):
        if abs(sig_weighted[now_index[i]])<0.2*mean_height:
            now_index[i]=-1
    now_index=np.setdiff1d(now_index,[-1])

    if show_rec:
        fig, ax = plt.subplots()
        fig.patch.set_alpha(0.)
        plt.subplot(111).plot(check_polarity(sig_weighted,F=True),'red')
        # slice = np.array([None] * len(Sig)).T
        # for i in range(5000,8600):
        #     slice[i]=Sig[i]
        # plt.subplot(111).plot(slice, 'mediumseagreen',linewidth=0.8)
        plt.axis('off')
        plt.show()

        # fig, ax = plt.subplots()
        # fig.patch.set_alpha(0.)
        # plt.subplot(111).plot(Sig[5000:8600], 'g', linewidth=0.5)
        # plt.axis('off')
        # plt.show()

        # fig, ax = plt.subplots()
        # fig.patch.set_alpha(0.)
        # plt.subplot(111).plot(sig_weighted,'red',linewidth=0.5)
        # slice = np.array([None] * len(sig_weighted)).T
        # for i in range(5000, 8600):
        #     slice[i] = sig_weighted[i]
        # # plt.subplot(111).plot(slice, 'g',linewidth=0.5)
        # plt.axis('off')
        # plt.show()
    rec=sig_weighted

    return now_index,rec
...
def local_renyi_en(sig):
    ...
    print(cmap)
    plt.pcolor(t, f, M, cmap=cmap)
    plt.colorbar()
    plt.show()
    # coeff = []
    # for i in range(M.shape[0]):
    #     temp = 1 - 4 * i / M.shape[0]
